...
export async function approveTask(
  ...
  try {
    ...
    // Create a safe new task object with required fields for the database
    const taskToCreate: Record<string, any> = {
      ...
      isAutoGenerated: false, // Key field: makes it a regular task
      needsApproval: false,   // Regular tasks don't need approval
      parentTaskId: task.id,  // Link to the specific auto-generated task
      createdAt: new Date(),
      isCanceled: false
    };
    
    // Add client reference if it exists
    if (task.clientId !== undefined && task.clientId !== null) {
      taskToCreate.clientId = task.clientId;
    }
    
    // Add entity reference if it exists
    ...
    if (task.serviceTypeId !== undefined && task.serviceTypeId !== null) {
      taskToCreate.serviceTypeId = task.serviceTypeId;
    }
    
    // Add category if it exists
    if (task.taskCategoryId !== undefined && task.taskCategoryId !== null) {
      taskToCreate.taskCategoryId = task.taskCategoryId;
    }
    
    // Add compliance fields if they exist
    if (task.complianceFrequency) taskToCreate.complianceFrequency = task.complianceFrequency;
    if (task.complianceYear) taskToCreate.complianceYear = task.complianceYear;
    if (task.complianceDuration) taskToCreate.complianceDuration = task.complianceDuration;
    if (task.complianceStartDate) taskToCreate.complianceStartDate = task.complianceStartDate;
    if (task.complianceEndDate) taskToCreate.complianceEndDate = task.complianceEndDate;
    if (task.compliancePeriod) taskToCreate.compliancePeriod = task.compliancePeriod;
    
    // Add financial fields if they exist
    if (task.currency) taskToCreate.currency = task.currency;
    if (task.serviceRate !== undefined) taskToCreate.serviceRate = task.serviceRate;
    if (task.invoiceId !== undefined && task.invoiceId !== null) {
      taskToCreate.invoiceId = task.invoiceId;
    }
    
    // Create the task with proper type safety
    console.log(`[APPROVAL] Creating regular task with parentTaskId = ${task.id}`);
    try {
      const newTaskId = await storage.createTask(taskToCreate);
      ...
      console.log(`[APPROVAL] Successfully created regular task with ID ${newTaskId}`);
    } catch (error) {

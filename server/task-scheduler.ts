...
export class TaskScheduler {
   ...
   * This is where we generate auto-generated tasks from recurring tasks
   */
  private async processRecurringTask(task: Task, leadDays: number): Promise<void> {
    ...
    // IMPORTANT: We need to start from the NEXT period AFTER the original task's compliance period
    // This ensures we generate tasks for future periods, not the current one
    const originalEndDate = new Date(task.complianceEndDate);
    
    // Calculate a reference date that is one day after the original end date
    // This ensures we start generating from the next period
    const nextDayAfterOriginalEnd = new Date(originalEndDate);
    nextDayAfterOriginalEnd.setDate(nextDayAfterOriginalEnd.getDate() + 1);
    
    console.log(`Original compliance period ends: ${format(originalEndDate, 'yyyy-MM-dd')}`);
    console.log(`Using reference date for next period: ${format(nextDayAfterOriginalEnd, 'yyyy-MM-dd')}`);
    
    // Start reference from day after original end date
    let referenceDate = nextDayAfterOriginalEnd;
    
    // Get current date as the upper limit (for future tasks, we'll check lead time)
    const now = new Date();
    
    // Add lead days to determine the cutoff point for future task generation
    ...
    // Generate sequence of periods from after original end date to current month + lead days
    console.log(`Generating periods from ${format(referenceDate, 'yyyy-MM')} to current month ${format(now, 'yyyy-MM')}`);
    ...
    for (let i = 0; i < 24; i++) {
      ...
      // Compare month/year of this period with cutoff date
      const periodMonthYear = format(firstDayOfMonth, 'yyyy-MM');
      
      // Check if this period is beyond our lead days cutoff
      // This ensures we only generate tasks up to the current month + lead days
      if (isAfter(firstDayOfMonth, cutoffDate)) {
        console.log(`Stopping at period ${periodMonthYear} as it's beyond cutoff date ${format(cutoffDate, 'yyyy-MM-dd')}`);
        break;
      }
      
      // Check if this period already exists as an auto-generated task
      ...
      if (!periodExists) {
        ...
        console.log(`Added period ${format(firstDayOfMonth, 'yyyy-MM-dd')} to ${format(lastDayOfMonth, 'yyyy-MM-dd')} to generation queue`);
      } else {
        console.log(`Period ${format(firstDayOfMonth, 'yyyy-MM-dd')} to ${format(lastDayOfMonth, 'yyyy-MM-dd')} already exists - skipping`);
      }
      
      // Move to the next period using the end date of the current period
  ...
  private async createRecurringTaskInstance(
    ...
    try {
      console.log(`Creating recurring task instance for template task ${templateTask.id}`);
      console.log(`Template task details: client: ${templateTask.clientId}, entity: ${templateTask.entityId}, service: ${templateTask.serviceTypeId}`);
      ...
      // Log the task data we're about to create
  ...
  public async approveTask(taskId: number, tenantId: number): Promise<boolean> {
    try {
      ...
      const regularTaskData: InsertTask = {
        ...
        parentTaskId: task.parentTaskId, // Link to the ORIGINAL parent, not the auto-generated task
  ...
  private calculateNextCompliancePeriod(
    ...
    try {
      ...
      switch (frequency.toLowerCase()) {
        ...
        case 'monthly':
          // For monthly tasks, always calculate the NEXT month after the reference date
          // This ensures we're always advancing forward in time and not stuck on the current month
          
          console.log(`Calculating next monthly period after: ${format(referenceDate, 'yyyy-MM-dd')}`);
          
          // Calculate the next month after the reference date
          ...
          // Using Date(year, month+1, 0) gives the last day of the month
          endDate = new Date(nextMonthYear, nextMonthIdx + 1, 0);
          endDate.setHours(23, 59, 59, 999);
          
          console.log(`Monthly task - Next period is: ${format(startDate, 'yyyy-MM-dd')} to ${format(endDate, 'yyyy-MM-dd')}`);
          
          // Validate that the dates make sense
          if (startDate <= referenceDate) {
            console.error(`Calculated start date (${format(startDate, 'yyyy-MM-dd')}) is not after reference date (${format(referenceDate, 'yyyy-MM-dd')})`);
            
            // Force next month to be 1 month from the reference date
            startDate = addMonths(startOfMonth(referenceDate), 1);
            endDate = endOfMonth(startDate);
            endDate.setHours(23, 59, 59, 999);
            
            console.log(`Corrected monthly task period: ${format(startDate, 'yyyy-MM-dd')} to ${format(endDate, 'yyyy-MM-dd')}`);
          }
          break;

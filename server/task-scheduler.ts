...
export class TaskScheduler {
  ...
  public async getTasksNeedingApproval(tenantId: number): Promise<Task[]> {
    try {
      ...
      // Critical: The filter must ensure that only tasks with BOTH isAutoGenerated=true AND needsApproval=true
  ...
  public async approveTask(taskId: number, tenantId: number): Promise<boolean> {
    try {
      // Get the specific task to be approved by its ID
      ...
      if (task.parentTaskId) {
        ...
        // 3. Are still auto-generated and not the current task
      ...
      try {
        ...
        // CRITICAL FIX: When approving a task, we need to create a new task in the main Tasks module
        // rather than trying to convert the existing auto-generated task
        // This ensures a complete separation of tasks between modules
        
        // First, create a clone of the current task as a regular task
        const newRegularTask = {
          ...
          // Critical differences to make it a regular task:
          ...
          parentTaskId: task.parentTaskId, // Maintain the lineage by keeping the parent task reference
          ...
          // Use fixed compliance end date if available
          complianceEndDate: fixedComplianceEndDate || task.complianceEndDate,
        ...
        // Now instead of updating the existing task, we'll just mark it as no longer 
        // needing approval since it's been processed
        const taskUpdate = {
          needsApproval: false, // Mark as processed
          updatedAt: new Date()  // Track when it was processed
        };
        
        // Only add the complianceEndDate if we have a valid date
        if (fixedComplianceEndDate) {
          taskUpdate.complianceEndDate = fixedComplianceEndDate;
        }
        
        // CRITICAL FIX: Instead of just updating the existing task,
        // we're going to create a brand new regular task and then mark the auto-generated
        // task as processed. This ensures a complete separation.
        
        console.log(`Creating new regular task based on auto-generated task ${taskId}`);
        const newTask = await this.storage.createTask(newRegularTask);
        ...
        console.log(`Created new regular task ${newTask.id} from auto-generated task ${taskId}`);
        
        // Now update the original auto-generated task to mark it as processed
        // but keep it as an auto-generated task (isAutoGenerated remains true)
        console.log(`Marking auto-generated task ${taskId} as processed:`, JSON.stringify(taskUpdate, null, 2));
        
        const updated = await this.storage.updateTask(taskId, taskUpdate);
        
        if (!updated) {
          console.log(`Warning: Failed to mark auto-generated task ${taskId} as processed`);
          // Continue anyway since we already created the regular task
        } else {
          console.log(`Successfully marked auto-generated task ${taskId} as processed`);
        }

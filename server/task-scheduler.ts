...
export class TaskScheduler {
   ...
   * When a task is approved, needsApproval is set to false
   * @param taskId - The ID of the task to approve
   * @param tenantId - Tenant ID for validation
   * @returns true if successful, false if not found or not eligible
   */
  public async approveTask(taskId: number, tenantId: number): Promise<boolean> {
    try {
      ...
      // Get the task to make sure it exists and belongs to the tenant
      const task = await this.storage.getTask(taskId);
      
      if (!task || task.tenantId !== tenantId) {
        console.log(`Task ${taskId} not found or does not belong to tenant ${tenantId}`);
        return false;
      }
      
      // Verify that this is an auto-generated task that needs approval
      if (!task.isAutoGenerated || !task.needsApproval) {
        console.log(`Task ${taskId} is not an auto-generated task or does not need approval`);
        return false;
      }
      
      // Update task to mark it as approved
      const updateData = {
        needsApproval: false,
        updatedAt: new Date()
      };

...
export class TaskScheduler {
   ...
   * Enforces strict filtering: is_auto_generated=TRUE AND needs_approval=TRUE
   */
   ...
   * Shows approved tasks from the auto-generated tasks module
   */
  ...
  public async approveTask(taskId: number, tenantId: number): Promise<boolean> {
    try {
      ...
      // 3. Determine if this is the most recent auto-generated task for this template
      // Get all auto-generated tasks with the same parent
      const allTasks = await this.storage.getTasks(tenantId);
      const relatedTasks = allTasks.filter(task => 
        task.parentTaskId === approvedTask.parentTaskId && 
        task.isAutoGenerated === true &&
        task.id !== approvedTask.id // Exclude the current task
      );
      
      // Sort by compliance end date (most recent last)
      relatedTasks.sort((a, b) => {
        const dateA = a.complianceEndDate ? new Date(a.complianceEndDate).getTime() : 0;
        const dateB = b.complianceEndDate ? new Date(b.complianceEndDate).getTime() : 0;
        return dateA - dateB;
      });
      
      // Check if this task has the most recent compliance period
      const currentTaskEndDate = approvedTask.complianceEndDate ? new Date(approvedTask.complianceEndDate).getTime() : 0;
      const isLatestTask = relatedTasks.every(task => {
        const taskEndDate = task.complianceEndDate ? new Date(task.complianceEndDate).getTime() : 0;
        return currentTaskEndDate >= taskEndDate;
      });
      
      console.log(`Task ${approvedTask.id} is${isLatestTask ? '' : ' NOT'} the most recent compliance period`);

      // 4. Prepare Updates for the Approved Task (to make IT the new recurring master if it's the latest)
      const approvedTaskUpdates = { // Using plain object to avoid type issues
        isRecurring: isLatestTask, // Only set to true if it's the latest compliance period
      ...
      // 5. Perform the Update on the Approved Task (IN PLACE)
      ...
      // 6. Prepare Updates for the Original Parent Task
      const parentTaskUpdates = { // Using plain object to avoid type issues
        isRecurring: false,
        // statusId: ID_OF_SUPERSEDED_STATUS, // Optional: Consider adding status update logic
        updatedAt: new Date(),
      };

      // 7. Perform the Update on the Original Parent Task (IN PLACE)

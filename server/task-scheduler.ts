...
export class TaskScheduler {
  ...
  public async approveTask(taskId: number, tenantId: number): Promise<boolean> {
    try {
      ...
      // Check if this specific task already exists as a regular task
      // We're only looking for a direct match with the same ID as parent to avoid
      // converting multiple tasks with the same compliance period
      const regularTaskExists = existingTasks.some(existingTask => {
        ...
        // ONLY check for a direct parent relationship with THIS specific task ID
        // This ensures we only match the exact task being approved by its ID
        // and not any task with similar client/entity/period
      ...
      try {
        ...
        // Create the update object with proper type safety

...
export class TaskScheduler {
  ...
  private async processRecurringTask(task: Task, leadDays: number): Promise<void> {
    ...
    // Get reference date - prefer compliance dates if available
    let referenceDate: Date;
    
    if (task.complianceEndDate) {
      // If we have a compliance end date, use that as the reference to ensure
      // we generate the next period correctly
      referenceDate = new Date(task.complianceEndDate);
    } else if (task.complianceStartDate) {
      referenceDate = new Date(task.complianceStartDate);
    } else {
      // Fallback to original task's created date or current date
      referenceDate = task.createdAt ? new Date(task.createdAt) : new Date();
    }
    
    // We need to generate all missing periods between the reference date and now
    // First, calculate the next period after the reference date
    let periodsToGenerate: { startDate: Date, endDate: Date, dueDate: Date }[] = [];
    let currentPeriodStart = referenceDate;
    const now = new Date();
    
    // Generate a sequence of periods until we reach the current date (plus buffer)
    for (let i = 0; i < 24; i++) { // Limit to 24 iterations to prevent infinite loops
      ...
      // Stop if we're generating periods beyond 60 days from now (buffer for future periods)
      if (isAfter(startDate, addDays(now, 60))) {
        console.log(`Stopping period generation as we've reached 60 days beyond current date`);
        break;
      }
      ...
      if (!periodExists) {
        periodsToGenerate.push({ startDate, endDate, dueDate });
      }
      
      // Update reference for next period
    ...
    for (const period of periodsToGenerate) {
      ...
      // Lead time logic - always generate if leadDays is 0 (manual generation)
      if (leadDays === 0) {
      ...
      if (isAfter(startDate, now)) {
        ...
        // For current or past periods, always generate if they don't exist
  ...
  private doesPeriodExist(task: Task, startDate: Date, endDate: Date, existingTasks: Task[]): boolean {
    return existingTasks.some(existingTask => {
      ...
      const existingEndDate = new Date(existingTask.complianceEndDate);
      
      // Check if the start and end dates match (same day)
      const startMatch = existingStartDate.toDateString() === startDate.toDateString();
      const endMatch = existingEndDate.toDateString() === endDate.toDateString();
      ...
      const statusMatch = (
        ...
        (existingTask.isAutoGenerated && !existingTask.needsApproval) ||
        // Or it's a regular task for this period

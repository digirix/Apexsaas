...
export class TaskScheduler {
  ...
  public async generateRecurringTasksForTenant(tenantId: number): Promise<void> {
    try {
      ...
      // Now handle future periods if within lead time
      if (leadDays > 0) {
        const now = new Date();
        // Check if we should generate next month's tasks based on lead time
        for (const task of recurringTasks) {
          try {
            // Skip one-time tasks
            ...
            // Only generate future tasks if lead days is configured
            // Get reference date - prefer compliance dates
            ...
            // Calculate the next period (one month ahead)
            ...
            // Check if it's within lead time
            const leadTimeThreshold = addDays(dueDate, -leadDays);
            
            if (isAfter(now, leadTimeThreshold)) {
              console.log(`Generating future task for ${task.id} - within lead time`);
            ...
            console.error(`Error processing future task ${task.id}:`, error);
          }
        }
      }
  ...
  public async approveTask(taskId: number, tenantId: number): Promise<boolean> {
    try {
      ...
      if (task.complianceEndDate) {
        ...
        const endDateMonth = new Date(task.complianceEndDate).getMonth();
        const endDateYear = new Date(task.complianceEndDate).getFullYear();
        
        // Last day of month at 23:59:59.999
        endDate = new Date(endDateYear, endDateMonth + 1, 0);

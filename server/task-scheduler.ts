    ...
    try {
      ...
      try {
        ...
        const autoTaskUpdate: any = {
          ...
          updatedAt: new Date() // Set updatedAt to track when it was processed
        };
        
        // Update only the compliance end date if we have it
        if (fixedComplianceEndDate) {
          autoTaskUpdate.complianceEndDate = fixedComplianceEndDate;
        }
        
        console.log(`Updating auto-generated task ${taskId} with data:`, JSON.stringify(autoTaskUpdate, null, 2));
        
        // Update the auto-generated task to mark it as processed but KEEP IT auto-generated
        await this.storage.updateTask(taskId, autoTaskUpdate);
        ...
        if (task.clientId !== null) newRegularTask.clientId = task.clientId;
        if (task.entityId !== null) newRegularTask.entityId = task.entityId;
        if (task.serviceTypeId !== null) newRegularTask.serviceTypeId = task.serviceTypeId;
        if (task.taskCategoryId !== null) newRegularTask.taskCategoryId = task.taskCategoryId;
        if (task.complianceFrequency) newRegularTask.complianceFrequency = task.complianceFrequency;
        if (task.complianceYear) newRegularTask.complianceYear = task.complianceYear;
        if (task.complianceDuration) newRegularTask.complianceDuration = task.complianceDuration;

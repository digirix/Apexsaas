...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/v1/finance/chart-of-accounts/sub-element-groups/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      // If elementGroupId is being changed, validate it
      if (data.elementGroupId) {
        const elementGroup = await storage.getChartOfAccountsElementGroup(data.elementGroupId, tenantId);
        if (!elementGroup) {
          return res.status(400).json({ message: "Invalid element group ID" });
        }
      ...
      const updatedGroup = await storage.updateChartOfAccountsSubElementGroup(id, tenantId, data);
  ...
  app.delete("/api/v1/finance/chart-of-accounts/sub-element-groups/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      // Check if there are detailed groups using this sub-element group
      const detailedGroups = await storage.getChartOfAccountsDetailedGroups(tenantId, id);
      if (detailedGroups.length > 0) {
        return res.status(400).json({ 
          message: "Cannot delete sub-element group that is used by detailed groups. Remove all detailed groups first." 
        });
      }
      
      const result = await storage.deleteChartOfAccountsSubElementGroup(id, tenantId);
      if (!result) {
        return res.status(500).json({ message: "Failed to delete sub-element group" });
      }
  ...
  app.patch("/api/v1/finance/chart-of-accounts/detailed-groups/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      // If subElementGroupId is being changed, validate it
      if (data.subElementGroupId) {
        const subElementGroup = await storage.getChartOfAccountsSubElementGroup(data.subElementGroupId, tenantId);
        if (!subElementGroup) {
          return res.status(400).json({ message: "Invalid sub-element group ID" });
        }
      ...
      const updatedGroup = await storage.updateChartOfAccountsDetailedGroup(id, tenantId, data);
      res.json(updatedGroup);
    } catch (error) {
  ...
  app.delete("/api/v1/finance/chart-of-accounts/detailed-groups/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      // Check if there are accounts using this detailed group
      const accounts = await storage.getChartOfAccounts(tenantId, undefined, id);
      if (accounts.length > 0) {
        return res.status(400).json({ 
          message: "Cannot delete detailed group that is used by accounts. Remove all accounts first." 
        });
      }

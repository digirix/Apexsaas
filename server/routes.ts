...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/v1/members/:id", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
      const id = parseInt(req.params.id);
  ...
  app.post("/api/v1/members", isAuthenticated, async (req, res) => {
    try {
      ...
      // Verify if member email already exists
      const existingMember = await storage.getUserByEmail(memberData.email, tenantId);
      if (existingMember) {
      ...
      // Add tenant ID and hashed password
      const newMemberData = {
        ...memberData,
        tenantId,
        password: hashedPassword,
        isSuperAdmin: false, // Only regular members can be created through this API
      };
      
      // Validate data
      const validatedData = insertUserSchema.parse(newMemberData);
      const newMember = await storage.createUser(validatedData);
  ...
  app.put("/api/v1/members/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      // Don't allow changing isSuperAdmin or tenantId
      delete memberData.isSuperAdmin;
      delete memberData.tenantId;
      
      const updatedMember = await storage.updateUser(id, memberData);
  ...
  app.post("/api/v1/members/:userId/permissions", isAuthenticated, async (req, res) => {
    try {
      ...
      const existingPermission = await storage.getModulePermission(userId, permissionData.module);
      if (existingPermission) {
        return res.status(400).json({ message: "Permission for this module already exists" });
      }
      
      // Set CRUD permissions according to accessLevel if not provided
      if (permissionData.accessLevel === 'full') {
        permissionData.canCreate = true;
        permissionData.canRead = true;
        permissionData.canUpdate = true;
        permissionData.canDelete = true;
      } else if (permissionData.accessLevel === 'restricted') {
      ...
      const newPermission = await storage.createModulePermission(newPermissionData);
  ...
  app.put("/api/v1/members/:userId/permissions/:permissionId", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
      const userId = parseInt(req.params.userId);
      ...
      const allPermissions = await storage.getUserPermissions(userId, tenantId);
      ...
      // Set CRUD permissions according to accessLevel if provided
      if (permissionData.accessLevel === 'full') {
        permissionData.canCreate = true;
        permissionData.canRead = true;
        permissionData.canUpdate = true;
      ...
      // Don't allow changing userId or tenantId
      delete permissionData.userId;
      delete permissionData.tenantId;
      
      const updatedPermission = await storage.updateModulePermission(permissionId, permissionData);
  ...
  app.delete("/api/v1/members/:userId/permissions/:permissionId", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
      const userId = parseInt(req.params.userId);
      const permissionId = parseInt(req.params.permissionId);
      ...
      await storage.deleteModulePermission(permissionId);
      res.status(204).send();
    } catch (error) {

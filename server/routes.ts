...
// Import the fixed TaskScheduler implementation
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Auto-generated tasks routes
  app.get("/api/v1/auto-tasks", isAuthenticated, async (req, res) => {
    try {
      ...
      const clientId = req.query.clientId ? parseInt(req.query.clientId as string) : undefined;
      const entityId = req.query.entityId ? parseInt(req.query.entityId as string) : undefined;
      const needsApproval = req.query.needsApproval === 'true' ? true : 
                           req.query.needsApproval === 'false' ? false : undefined;
      
      const tasks = await storage.getAutoGeneratedTasks(tenantId, clientId, entityId, needsApproval);
      
      return res.status(200).json(tasks);
    } catch (error) {
      console.error("Error fetching auto-generated tasks:", error);
      return res.status(500).json({ error: "Failed to fetch auto-generated tasks" });
    }
  });

  app.get("/api/v1/auto-tasks/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      const task = await storage.getAutoGeneratedTask(taskId, tenantId);
      
      if (!task) {
        return res.status(404).json({ error: "Auto-generated task not found" });
      }
      
      return res.status(200).json(task);
    } catch (error) {
      console.error("Error fetching auto-generated task:", error);
      return res.status(500).json({ error: "Failed to fetch auto-generated task" });
    }
  });

  app.post("/api/v1/auto-tasks/:id/approve", isAuthenticated, async (req, res) => {
    try {
      const taskId = parseInt(req.params.id);
      const tenantId = req.user?.tenantId;
      ...
      console.error("Error approving auto-generated task:", error);
      return res.status(500).json({ error: "Failed to approve task" });
    }
  });

  app.post("/api/v1/auto-tasks", isAuthenticated, async (req, res) => {
    try {
      const tenantId = req.user?.tenantId;
      ...
      // Ensure the tenant ID in the request body matches the user's tenant ID
      req.body.tenantId = tenantId;
      
      const task = await storage.createAutoGeneratedTask(req.body);
      
      return res.status(201).json(task);
    } catch (error) {
      console.error("Error creating auto-generated task:", error);
      return res.status(500).json({ error: "Failed to create auto-generated task" });
    }
  });

  app.put("/api/v1/auto-tasks/:id", isAuthenticated, async (req, res) => {
    try {
      const taskId = parseInt(req.params.id);
      const tenantId = req.user?.tenantId;
      ...
      // Get the task to verify it belongs to the user's tenant
      const existingTask = await storage.getAutoGeneratedTask(taskId, tenantId);
      
      if (!existingTask) {
        return res.status(404).json({ error: "Auto-generated task not found" });
      }
      
      const updatedTask = await storage.updateAutoGeneratedTask(taskId, req.body);
      
      if (!updatedTask) {
        return res.status(404).json({ error: "Auto-generated task not found or could not be updated" });
      }
      
      return res.status(200).json(updatedTask);
    } catch (error) {
      console.error("Error updating auto-generated task:", error);
      return res.status(500).json({ error: "Failed to update auto-generated task" });
    }
  });

  app.delete("/api/v1/auto-tasks/:id", isAuthenticated, async (req, res) => {
    try {
      const taskId = parseInt(req.params.id);
      const tenantId = req.user?.tenantId;
      ...
      const success = await storage.deleteAutoGeneratedTask(taskId, tenantId);
      
      if (!success) {
        return res.status(404).json({ error: "Auto-generated task not found or could not be deleted" });
      }
      
      return res.status(200).json({ success: true, message: "Auto-generated task deleted successfully" });
    } catch (error) {
      console.error("Error deleting auto-generated task:", error);
      return res.status(500).json({ error: "Failed to delete auto-generated task" });
    }
  });
  ...
  app.post("/api/v1/auto-generated-tasks/approve-all", isAuthenticated, async (req, res) => {
    try {
      ...
      const pendingTasks = await storage.getAutoGeneratedTasksByApprovalStatus(tenantId, true);
      ...
      // Process each task individually, one at a time with our improved function
      for (const task of pendingTasks) {
        try {
          // Use our improved approval function that handles one task at a time properly
          const approved = await approveTask(storage, task.id, tenantId);
      ...
      res.json({ 
        message: `Approved ${approvedCount} out of ${pendingTasks.length} pending tasks successfully` 
      });
    } catch (error) {
  ...
  // Debug route for approving a task without authentication (for testing only)
  app.post("/api/v1/debug/approve-task", async (req, res) => {
    try {
      const { taskId, tenantId } = req.body;
      
      if (!taskId || !tenantId) {
        return res.status(400).json({ message: "taskId and tenantId are required" });
      }
      
      // Get task before approval
      const taskBefore = await storage.getTask(taskId, tenantId);
      if (!taskBefore) {
        return res.status(404).json({ message: "Task not found" });
      }
      
      console.log(`DEBUG: Task before approval:`, JSON.stringify(taskBefore, null, 2));
      
      // Check tasks before approval
      const tasksBefore = await storage.getTasks(tenantId);
      console.log(`DEBUG: Found ${tasksBefore.length} tasks before approval`);
      console.log(`DEBUG: Auto-generated tasks before:`, tasksBefore.filter(t => t.isAutoGenerated).length);
      console.log(`DEBUG: Regular tasks before:`, tasksBefore.filter(t => !t.isAutoGenerated).length);
      
      // Use the TaskScheduler implementation for approval
      const taskScheduler = new TaskScheduler(storage);
      const success = await taskScheduler.approveTask(taskId, tenantId);
      
      // Get tasks after approval
      const tasksAfter = await storage.getTasks(tenantId);
      console.log(`DEBUG: Found ${tasksAfter.length} tasks after approval`);
      console.log(`DEBUG: Auto-generated tasks after:`, tasksAfter.filter(t => t.isAutoGenerated).length);
      console.log(`DEBUG: Regular tasks after:`, tasksAfter.filter(t => !t.isAutoGenerated).length);
      
      // Get the approved task after approval to show its flags
      const taskAfter = await storage.getTask(taskId, tenantId);
      console.log(`DEBUG: Task after approval:`, JSON.stringify(taskAfter, null, 2));
      
      // Get the parent task to show its flags
      if (taskBefore.parentTaskId) {
        const parentTaskAfter = await storage.getTask(taskBefore.parentTaskId, tenantId);
        console.log(`DEBUG: Parent task after approval:`, JSON.stringify(parentTaskAfter, null, 2));
      }
      
      res.json({
        success,
        message: success ? "Task approved successfully" : "Failed to approve task",
        tasksBefore: tasksBefore.length,
        tasksAfter: tasksAfter.length,
        task: taskAfter
      });
    } catch (error) {
      console.error("Error in debug approve route:", error);
      res.status(500).json({ 
  ...
  // Debug route to get stats on auto-generated tasks (for testing only)
  app.get("/api/v1/debug/tasks", async (req, res) => {
    try {
      const tenantId = parseInt(req.query.tenantId as string) || 5;
      
      const allTasks = await storage.getTasks(tenantId);
      const autoGeneratedTasks = allTasks.filter(t => t.isAutoGenerated === true);
      const regularTasks = allTasks.filter(t => t.isAutoGenerated === false || t.isAutoGenerated === null);
      const recurringTasks = allTasks.filter(t => t.isRecurring === true);
      ...
      res.json({
        ...
        stats: {
          ...
          needsApproval: autoGeneratedTasks.filter(t => t.needsApproval === true).length
        },
        autoGeneratedTasks: autoGeneratedTasks.map(t => ({
          id: t.id,
          parentTaskId: t.parentTaskId,
          isAutoGenerated: t.isAutoGenerated,
          isRecurring: t.isRecurring,
          needsApproval: t.needsApproval,
          compliancePeriod: t.compliancePeriod
        }))
      });
    } catch (error) {
      console.error("Error in debug tasks route:", error);
      res.status(500).json({ 
  ...
  // Debug route to test task approval (for testing only)
  app.get("/api/v1/debug/test-approval/:taskId", async (req, res) => {
    try {
      const taskId = parseInt(req.params.taskId);
      const tenantId = 5; // Default tenant for testing
      ...
      // Use the TaskScheduler implementation for approval
      const taskScheduler = new TaskScheduler(storage);
      console.log(`*** TESTING FIXED APPROVAL IMPLEMENTATION: Approving task ${taskId} ***`);
      const success = await taskScheduler.approveTask(taskId, tenantId);
      
      // Get tasks after approval
      const tasksAfter = await storage.getTasks(tenantId);
      
      // Get approved task after to check flags
      const taskAfter = await storage.getTask(taskId, tenantId);
      
      // Get parent task after to check flags
      let parentTaskAfter = null;
      if (taskBefore.parentTaskId) {
        parentTaskAfter = await storage.getTask(taskBefore.parentTaskId, tenantId);
      }
      ...
      res.json({
        ...
        afterCount: tasksAfter.length,
        taskBefore: {
          id: taskBefore.id,
          isAutoGenerated: taskBefore.isAutoGenerated,
          isRecurring: taskBefore.isRecurring,
          needsApproval: taskBefore.needsApproval,
          parentTaskId: taskBefore.parentTaskId
        },
        taskAfter: taskAfter ? {
          id: taskAfter.id,
          isAutoGenerated: taskAfter.isAutoGenerated,
          isRecurring: taskAfter.isRecurring,
          needsApproval: taskAfter.needsApproval,
          parentTaskId: taskAfter.parentTaskId
        } : null,
        parentBefore: taskBefore.parentTaskId ? {
          id: taskBefore.parentTaskId,
          isRecurring: tasksBefore.find(t => t.id === taskBefore.parentTaskId)?.isRecurring
        } : null,
        parentAfter: parentTaskAfter ? {
          id: parentTaskAfter.id,
          isRecurring: parentTaskAfter.isRecurring
        } : null
      });
    } catch (error) {
      console.error("Error in debug test approval route:", error);
      res.status(500).json({
        message: "Error testing approval",
        error: error instanceof Error ? error.message : String(error)
      });
    }
  });
  ...
  app.get("/api/v1/debug/test-approval/:taskId", async (req, res) => {
    try {
      ...
      // Get parent task before approval
      let parentTaskBefore = null;
      if (taskBefore.parentTaskId) {
        parentTaskBefore = await storage.getTask(taskBefore.parentTaskId, tenantId);
        console.log(`DEBUG: Parent task before approval:`, JSON.stringify(parentTaskBefore, null, 2));
      }
      
      // Get all tasks before approval
      const tasksBefore = await storage.getTasks(tenantId);
      const beforeCount = tasksBefore.length;
      console.log(`DEBUG: Found ${beforeCount} tasks before approval`);
      
      // Use the TaskScheduler implementation for approval
      const taskScheduler = new TaskScheduler(storage);
      const success = await taskScheduler.approveTask(taskId, tenantId);
      
      // Get all tasks after approval to check if count changed
      const tasksAfter = await storage.getTasks(tenantId);
      const afterCount = tasksAfter.length;
      console.log(`DEBUG: Found ${afterCount} tasks after approval`);
      
      // Get the approved task after approval
      const taskAfter = await storage.getTask(taskId, tenantId);
      console.log(`DEBUG: Task after approval:`, JSON.stringify(taskAfter, null, 2));
      
      // Get the parent task after approval
      let parentTaskAfter = null;
      if (taskBefore.parentTaskId) {
        parentTaskAfter = await storage.getTask(taskBefore.parentTaskId, tenantId);
        console.log(`DEBUG: Parent task after approval:`, JSON.stringify(parentTaskAfter, null, 2));
      ...
      res.json({
        ...
        parentBefore: parentTaskBefore ? {
          id: parentTaskBefore.id,
          isRecurring: parentTaskBefore.isRecurring
        } : null,
        parentAfter: parentTaskAfter ? {
          id: parentTaskAfter.id,
          isRecurring: parentTaskAfter.isRecurring
  ...
  // Debug route to create test tasks for approval testing
  app.post("/api/v1/debug/create-test-tasks", async (req, res) => {
    try {
      const tenantId = 5; // Default tenant for testing
      ...
      const parentTask = {
        ...
        taskCategoryId: 2, 
        assigneeId: 5,
        dueDate: new Date("2025-05-31T23:59:59.999Z"),
        statusId: 2,
        taskDetails: "Parent Recurring Task",
        nextToDo: "",
        isRecurring: true,
        complianceFrequency: "Monthly",
        complianceYear: "2025",
        complianceDuration: "",
        complianceStartDate: new Date("2025-05-01T00:00:00.001Z"),
        complianceEndDate: new Date("2025-05-31T23:59:59.999Z"),
        compliancePeriod: "May 2025",
        currency: "PKR",
        serviceRate: 100,
      ...
      const parent = await storage.createTask(parentTask);
      console.log(`Created parent task with ID: ${parent.id}`);
      ...
      const childTask = {
        ...
        taskCategoryId: 2,
        assigneeId: 5,
        dueDate: new Date("2025-06-30T23:59:59.999Z"),
        statusId: 2,
        taskDetails: "Auto-Generated Task",
        nextToDo: "",
        isRecurring: false,
        complianceFrequency: "Monthly",
        complianceYear: "2025",
        complianceDuration: "",
        complianceStartDate: new Date("2025-06-01T00:00:00.001Z"),
        complianceEndDate: new Date("2025-06-30T23:59:59.999Z"),
        compliancePeriod: "June 2025",
        currency: "PKR",
        serviceRate: 100,
      ...
      const child = await storage.createTask(childTask);
      console.log(`Created auto-generated task with ID: ${child.id}`);
      ...
      res.status(500).json({
        success: false,
        message: "Failed to create test tasks",
        error: error instanceof Error ? error.message : String(error)
      });
    }
  });

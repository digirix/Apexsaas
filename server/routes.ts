...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
  ...
  }); tasks after:`, tasksAfter.filter(t => t.isAutoGenerated).length);
      console.log(`DEBUG: Regular tasks after:`, tasksAfter.filter(t => !t.isAutoGenerated).length);
      
      // Get the task after approval
      const taskAfter = await storage.getTask(taskId, tenantId);
      if (taskAfter) {
        console.log(`DEBUG: Task after approval:`, JSON.stringify(taskAfter, null, 2));
      } else {
        console.log(`DEBUG: Task ${taskId} no longer exists after approval`);
      }
      ...
      if (success) {
        res.json({ 
          ...
          tasksBefore: tasksBefore.length,
          tasksAfter: tasksAfter.length,
          autoGeneratedBefore: tasksBefore.filter(t => t.isAutoGenerated).length,
          autoGeneratedAfter: tasksAfter.filter(t => t.isAutoGenerated).length,
          regularBefore: tasksBefore.filter(t => !t.isAutoGenerated).length,
          regularAfter: tasksAfter.filter(t => !t.isAutoGenerated).length
        });
      } else {
        res.status(404).json({ message: "Task not found or not eligible for approval" });
      }
    } catch (error) {
      console.error("Error in debug approve task:", error);
      res.status(500).json({ 
        message: "Failed to approve task",
        error: error instanceof Error ? error.message : String(error)
      });

...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/v1/tasks", isAuthenticated, async (req, res) => {
    try {
      ...
      // Get all tasks first
      const allTasks = await storage.getTasks(tenantId, clientId, entityId, isAdmin, categoryId, statusId);
      
      // CRITICAL FIX: Filter out auto-generated tasks that need approval
      // This ensures tasks don't appear in both the regular tasks list and auto-generated tasks list
      const filteredTasks = allTasks.filter(task => {
        // If a task is auto-generated AND needs approval, it should only appear in the auto-generated tasks list
        if (task.isAutoGenerated === true && task.needsApproval === true) {
          return false; // Exclude from regular tasks list
        }
        
        // All other tasks (regular tasks or processed auto-generated tasks) should appear in the regular tasks list
        return true;
      });
      
      console.log(`Filtered ${allTasks.length - filteredTasks.length} auto-generated tasks from regular tasks list`);

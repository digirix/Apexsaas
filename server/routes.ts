...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/v1/ai/configurations", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
  ...
  app.get("/api/v1/ai/active-configuration", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
      const configurations = await storage.getAiConfigurations(tenantId);
  ...
  app.post("/api/v1/ai/configurations", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
      const { id, provider, model, apiKey, isActive } = req.body;
      
      // If this is set to be the active config, deactivate all others
      if (isActive) {
        const allConfigs = await storage.getAiConfigurations(tenantId);
        for (const config of allConfigs) {
          if (config.isActive && config.id !== id) {
            await storage.updateAiConfiguration(config.id, { isActive: false });
          }
        }
      }
      
      // Use AiService to encrypt the API key
      
      // Encrypt API key
      const encryptedApiKey = apiKey ? AiService.encryptApiKey(apiKey) : undefined;
      
      let result;
      if (id) {
        // Update existing configuration
        const updateData: any = { model };
        if (encryptedApiKey) updateData.apiKey = encryptedApiKey;
        if (isActive !== undefined) updateData.isActive = isActive;
        
        result = await storage.updateAiConfiguration(id, updateData);
      } else {
        // Create new configuration
        result = await storage.createAiConfiguration({
          tenantId,
          provider,
          model,
          apiKey: encryptedApiKey as string,
          isActive: isActive !== undefined ? isActive : false
        });
      }
      
      // Don't return API key in response for security
      const { apiKey: _, ...safeResult } = result;
      res.status(id ? 200 : 201).json(safeResult);
    } catch (error) {
  ...
  app.post("/api/v1/ai/test-connection", isAuthenticated, async (req, res) => {
    try {
  ...
  app.post("/api/v1/ai/chat", isAuthenticated, async (req, res) => {
    try {
      ...
      if (conversationId) {
        ...
        const conversation = await storage.getAiChatConversation(parseInt(conversationId), tenantId);
        if (!conversation) {
          return res.status(404).json({ message: "Conversation not found" });
        }
        
        // Get conversation history
        conversationHistory = await storage.getAiChatMessages(conversation.id);
      } else {
        // Create new conversation
        const newConversation = await storage.createAiChatConversation({
          tenantId,
          userId,
      ...
      const response = await aiService.processChatMessage(message, conversationHistory);
  ...
  app.get("/api/v1/ai/chat-history/:conversationId", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
      const conversationId = parseInt(req.params.conversationId);
      
      // Verify conversation exists and belongs to tenant
      const conversation = await storage.getAiChatConversation(conversationId, tenantId);
      if (!conversation) {
        return res.status(404).json({ message: "Conversation not found" });
      }
      
      // Get messages
      const messages = await storage.getAiChatMessages(conversationId);
  ...
  app.post("/api/v1/ai/task-suggestions", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
      ...
      // Get entity type
      const entityType = await storage.getEntityType(entity.entityTypeId);
      
      // Get entity tax jurisdictions and services
      const taxJurisdictions = await storage.getEntityTaxJurisdictions(tenantId, entityId);
      const services = await storage.getEntityServiceSubscriptions(tenantId, entityId);
      
      // Prepare entity data for AI
      const entityData = {
        name: entity.name,
        type: entityType?.name || "Unknown",
        jurisdictions: taxJurisdictions.map(tj => tj.name),
        services: services.filter(s => s.isSubscribed).map(s => s.name),
        fiscalYearEnd: entity.fiscalYearEnd || "December 31",
        taxIds: entity.taxIdentifiers ? JSON.parse(entity.taxIdentifiers) : {}
      };
      ...
      const suggestions = await aiService.generateTaskSuggestions(entityData);
      
      // Save suggestions to database
      await storage.createAiTaskSuggestion({
        tenantId,
        entityId,
  ...
  app.post("/api/v1/ai/analyze-compliance", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
      const { entityId } = req.body;
      ...
      const analysis = await aiService.analyzeEntityCompliance(entityData);
      
      // Save analysis to database
      await storage.createAiComplianceAnalysis({
        tenantId,
        entityId,
        analysis
      });
      ...
      res.status(500).json({ 
        success: false, 
        error: error.message || "Failed to analyze entity compliance"
      });
    }
  });

  app.post("/api/v1/ai/extract-document", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
      const userId = (req.user as any).id;
      ...
      const existingAnalysis = await storage.getAiDocumentAnalysisByHash(tenantId, documentHash);
      if (existingAnalysis) {
        return res.json({
          success: true,
          extractedInfo: JSON.parse(existingAnalysis.extractedInfo as any),
          cached: true
        });
      }
      ...
      const extractedInfo = await aiService.extractDocumentInfo(documentType, documentContent);
      
      // Save to database
      await storage.createAiDocumentAnalysis({
        tenantId,
        userId,
        documentType,
        documentHash,
  ...
  app.post("/api/v1/tasks/bulk-create", isAuthenticated, async (req, res) => {
    try {
      const tenantId = (req.user as any).tenantId;
      const userId = (req.user as any).id;
      const { entityId, suggestions } = req.body;
      
      if (!entityId || !suggestions || !Array.isArray(suggestions)) {
        return res.status(400).json({ message: "Entity ID and task suggestions array are required" });
      }
      ...
      // Get task categories
      const categories = await storage.getTaskCategories(tenantId);
      
      // Get initial task status
      const statuses = await storage.getTaskStatuses(tenantId);
      const initialStatus = statuses.find(s => s.isInitial) || statuses[0];
      
      if (!initialStatus) {
        return res.status(400).json({ message: "No task status found to assign to new tasks" });
      }
      ...
      for (const suggestion of suggestions) {
        ...
        if (existingCategory) {
          ...
          // Create new category
          const newCategory = await storage.createTaskCategory({
            tenantId,
      ...
      // Record AI learning data about task creation
      await storage.createAiLearningData({
        tenantId,
        dataType: 'task_creation',
        data: JSON.stringify({
          entityId,
      ...
      res.status(500).json({ message: "Failed to create tasks from suggestions" });
    }
  });

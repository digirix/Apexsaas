...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/v1/auto-tasks/:id/approve", isAuthenticated, async (req, res) => {
    try {
      ...
      const success = await storage.approveAutoGeneratedTask(taskId, tenantId);
      
      if (!success) {
        return res.status(404).json({ error: "Auto-generated task not found or could not be approved" });
      }
  ...
  app.post("/api/v1/debug/approve-task", async (req, res) => {
    try {
      ...
      // Use the improved task approval implementation
      const { approveTask } = await import('./task-approval');
      const success = await approveTask(storage, taskId, tenantId);
      ...
      // Get the task after approval (if it still exists)

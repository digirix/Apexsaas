...
export class DatabaseStorage implements IStorage {
  ...
  async getTasks(tenantId?: number, clientId?: number, entityId?: number, statusId?: number): Promise<Task[]> {
    try {
      ...
      let query = db.select({
        ...
        compliancePeriod: tasks.compliancePeriod,
  ...
  async approveAutoGeneratedTask(id: number, tenantId: number): Promise<boolean> {
    try {
      // Import the task approval function from task-approval.ts
      const { approveTask } = await import('./task-approval');
      
      // Use the imported function to handle the approval logic
      return await approveTask(this, id, tenantId);
    } catch (error) {

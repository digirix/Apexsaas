...
export class DatabaseStorage implements IStorage {
  ...
  async approveAutoGeneratedTask(id: number, tenantId: number): Promise<boolean> {
    try {
      // Create an instance of the TaskScheduler to handle approval
      const { TaskScheduler } = await import('./task-scheduler');
      const scheduler = new TaskScheduler(this);
      
      // Use the TaskScheduler to handle the approval logic
      return await scheduler.approveTask(id, tenantId);
    } catch (error) {

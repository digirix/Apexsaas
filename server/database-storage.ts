...
import type { 
  ...
  // Auto-generated tasks types
  AutoGeneratedTask, InsertAutoGeneratedTask
} from "@shared/schema";
...
export class DatabaseStorage implements IStorage {
  ...
  // Auto-generated tasks operations
  async getAutoGeneratedTasks(
    tenantId: number, 
    clientId?: number, 
    entityId?: number, 
    ...
    try {
      let query = db.select().from(autoGeneratedTasks)
        .where(eq(autoGeneratedTasks.tenantId, tenantId));
      
      if (clientId !== undefined) {
        query = query.where(eq(autoGeneratedTasks.clientId, clientId));
      }
      
      if (entityId !== undefined) {
        query = query.where(eq(autoGeneratedTasks.entityId, entityId));
      }
      
      if (needsApproval !== undefined) {
        query = query.where(eq(autoGeneratedTasks.needsApproval, needsApproval));
      }
      
      return await query;
    } catch (error) {
      console.error('Error in getAutoGeneratedTasks:', error);
      return [];
    }
  }
  
  async getAutoGeneratedTasksByApprovalStatus(
    tenantId: number,
    needsApproval: boolean
  ): Promise<AutoGeneratedTask[]> {
    try {
      const query = db.select().from(autoGeneratedTasks)
        .where(and(
          eq(autoGeneratedTasks.tenantId, tenantId),
          eq(autoGeneratedTasks.needsApproval, needsApproval)
        ))
        .orderBy(asc(autoGeneratedTasks.dueDate)); // Order by due date to process oldest tasks first
      
      return await query;
    } catch (error) {
      console.error('Error in getAutoGeneratedTasksByApprovalStatus:', error);
      return [];
    }
  }

  async getAutoGeneratedTask(id: number, tenantId: number): Promise<AutoGeneratedTask | undefined> {
    try {
      const [task] = await db.select().from(autoGeneratedTasks)
        .where(and(
          eq(autoGeneratedTasks.id, id),
          eq(autoGeneratedTasks.tenantId, tenantId)
        ));
      return task;
    } catch (error) {
      console.error('Error in getAutoGeneratedTask:', error);
      return undefined;
    }
  }

  async createAutoGeneratedTask(task: InsertAutoGeneratedTask): Promise<AutoGeneratedTask> {
    try {
      // Set proper time formats for compliance dates
      const taskData = { ...task };
      
      if (taskData.complianceStartDate) {
        const startDate = new Date(taskData.complianceStartDate);
        startDate.setHours(0, 0, 0, 1); // 00:00:00.001
        taskData.complianceStartDate = startDate;
      }
      
      if (taskData.complianceEndDate) {
        const endDate = new Date(taskData.complianceEndDate);
        endDate.setHours(23, 59, 59, 999); // 23:59:59.999
        taskData.complianceEndDate = endDate;
      }
      
      // Calculate compliance period if not provided
      if (!taskData.compliancePeriod && taskData.complianceFrequency && taskData.complianceStartDate) {
        const { calculateCompliancePeriod } = await import('./task-approval');
        taskData.compliancePeriod = calculateCompliancePeriod(
          taskData.complianceFrequency, 
          new Date(taskData.complianceStartDate)
        );
      }
      
      const [newTask] = await db.insert(autoGeneratedTasks).values(taskData).returning();
      return newTask;
    } catch (error) {
      console.error('Error in createAutoGeneratedTask:', error);
      throw error;
    }
  }

  async updateAutoGeneratedTask(
    id: number, 
    task: Partial<InsertAutoGeneratedTask>
  ): Promise<AutoGeneratedTask | undefined> {
    try {
      // Set proper time formats for compliance dates
      const taskData = { ...task };
      ...
      // Update the task with proper data
      const [updatedTask] = await db.update(autoGeneratedTasks)
        .set(taskData)
        .where(eq(autoGeneratedTasks.id, id))
        .returning();
      
      return updatedTask;
    } catch (error) {
      console.error('Error in updateAutoGeneratedTask:', error);
      return undefined;
    }
  }

  async deleteAutoGeneratedTask(id: number, tenantId: number): Promise<boolean> {
    try {
      const [deletedTask] = await db.delete(autoGeneratedTasks)
        .where(and(
          eq(autoGeneratedTasks.id, id),
          eq(autoGeneratedTasks.tenantId, tenantId)
        ))
        .returning({ id: autoGeneratedTasks.id });
      
      return !!deletedTask;
    } catch (error) {
      console.error('Error in deleteAutoGeneratedTask:', error);
      return false;
    }
  }

  /**
   * Direct implementation of task approval to ensure we're not creating new tasks
   * This directly updates task flags and status without any intermediate steps
   * or creating any new records
   */
  async approveAutoGeneratedTask(id: number, tenantId: number): Promise<boolean> {
    try {
      console.log(`Processing approval directly for auto-generated task ${id}`);
      
      // First, get the task to approve
      const task = await this.getTask(id, tenantId);
      
      if (!task) {
        console.error(`Task ${id} not found for tenant ${tenantId}`);
        return false;
      }
      
      // Verify this is an auto-generated task that needs approval
      if (task.isAutoGenerated !== true || task.needsApproval !== true) {
        console.error(`Task ${id} is not an auto-generated task pending approval`);
        return false;
      }
      
      console.log(`Task details for approval: parentId=${task.parentTaskId}, isAutoGenerated=${task.isAutoGenerated}, isRecurring=${task.isRecurring}`);
      
      // Find the parent task
      if (!task.parentTaskId) {
        console.error(`Task ${id} has no parent task ID`);
        return false;
      }
      
      const parentTask = await this.getTask(task.parentTaskId, tenantId);
      if (!parentTask) {
        console.error(`Parent task ${task.parentTaskId} not found`);
        return false;
      }
      
      // CRITICAL: Update the auto-generated task itself, modifying it in place
      // 1. This task is now approved, so it's no longer auto-generated
      // 2. If it's the latest in the series, it becomes the new recurring source
      await this.updateTask(id, {
        isAutoGenerated: false,
        needsApproval: false,
        isRecurring: true, // This is now the new recurring source
        updatedAt: new Date()
      });
      
      // Update the parent task to no longer be recurring
      if (parentTask.isRecurring) {
        console.log(`Setting parent task ${parentTask.id} isRecurring to false (passing the baton)`);
        await this.updateTask(parentTask.id, {
          isRecurring: false,
          updatedAt: new Date()
        });
      }
      
      console.log(`Task ${id} has been directly approved and parent ${parentTask.id} was updated`);
      return true;
    } catch (error) {
      console.error('Error in approveAutoGeneratedTask:', error);
      return false;
    }
  }
}
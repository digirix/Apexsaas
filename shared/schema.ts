...
// Task Messages and Time Tracking Schema

// Task messages table for internal chat
...
// Task message mentions table
export const taskMessageMentions = pgTable("task_message_mentions", {
  id: serial("id").primaryKey(),
  taskMessageId: integer("task_message_id").notNull(),
  mentionedUserId: integer("mentioned_user_id").notNull(),
  tenantId: integer("tenant_id").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => {
  return {
    messageFk: foreignKey({ columns: [table.taskMessageId], foreignColumns: [taskMessages.id] }),
    mentionedUserFk: foreignKey({ columns: [table.mentionedUserId], foreignColumns: [users.id] }),
    tenantFk: foreignKey({ columns: [table.tenantId], foreignColumns: [tenants.id] })
  };
});
...
export const insertTaskMessageMentionSchema = createInsertSchema(taskMessageMentions).pick({
  taskMessageId: true,
  mentionedUserId: true,
  tenantId: true,
});

export const insertTaskTimeEntrySchema = createInsertSchema(taskTimeEntries).pick({
  taskId: true,
  userId: true,
  tenantId: true,
  startTime: true,
...
export type TaskMessageMention = typeof taskMessageMentions.$inferSelect;
export type InsertTaskMessageMention = z.infer<typeof insertTaskMessageMentionSchema>;

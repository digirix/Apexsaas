...
import { Bell, RefreshCw, Users, FileText, DollarSign, Workflow, Shield, Brain, AlertTriangle, ChevronDown, ChevronRight } from "lucide-react";
...
export function NotificationSettings() {
  ...
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set(['Tasks & Projects', 'Finance & Payments']));
  ...
  const totalEnabled = Object.values(NOTIFICATION_CATEGORIES).reduce(
    (acc, category) => acc + getEnabledCount(category.types), 0
  );
  const totalNotifications = Object.values(NOTIFICATION_CATEGORIES).reduce(
    (acc, category) => acc + category.types.length, 0
  );
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <CardDescription>
                  Control which notifications you receive ({totalEnabled} of {totalNotifications} enabled)
                </CardDescription>
              </div>
            </div>
            <Button 
          ...
          {Object.entries(NOTIFICATION_CATEGORIES).map(([categoryName, category]) => {
            ...
            return (
              <div key={categoryName} className="border rounded-lg">
                <div 
                  className="flex items-center justify-between cursor-pointer hover:bg-muted/50 p-3 transition-colors"
                          ...
                          <div key={notificationType.key} className="flex items-center justify-between py-2">
                            <div className="flex-1">
                              <div className="flex items-center space-x-2 mb-1">
                                <h5 className="text-sm font-medium">{notificationType.label}</h5>
                                {isEnabled && <Badge variant="outline" className="text-xs h-4">ON</Badge>}
                              </div>
